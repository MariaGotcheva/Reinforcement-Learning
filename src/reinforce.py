...
class REINFORCE:
    ...
    def get_action(self, curr_state, *args, **kwargs):
        ...
        for a in range(self.num_actions):
            l_list.append(np.dot(self.parameters, self.features(curr_state, a)))
    ...
    def compute_update(self, traj, timestep):
        ...
        return_t = sum(traj[t][2] for t in range(timestep, traj_len))
        curr_s, action, x, y, z = traj[timestep]

        feature = self.features(curr_s, action)

        l_list = []
        for a in range(self.num_actions):
            l_list.append(np.dot(self.parameters, self.features(curr_s, a)))
        ...
        weighted_f = []
        for a in range(self.num_actions):
            weighted_f.append(prob_dist[a] * self.features(curr_s, a))

        g_l = feature - sum(weighted_f)
        update = return_t * g_l
